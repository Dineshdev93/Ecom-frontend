...
import { Add_review_Slice, Delete_Review, Getting_Review } from "../../redux/slice/adminproductSlice/adminproductSlice";
        ...
        <Container>
          <Row className="justify-content-center">
            ...
            {productdata[0].SingleproductState?.map((item, indexs) => {
              return (
                <>
                  <Col md={6} key={indexs} className="product-details__image mb-4">
                    <img src={`${item.productimage}`} alt="Nike Shoes" width={500} />
                  </Col>
                  <Col md={6} className="product-details__info">
                    <h3 className="product-details__name">{item.productname}</h3>
                    <div className="product-details__ratings">
                      ...
                      >
                        ...
                        {/* Empty Stars */}
                           {Array.from({ length: 5 - showrating }).map((_, index) => (
                             <i key={index + showrating} className="fa-regular fa-star"></i>
                            ))}
                      </span>
                      <span
          ...
          <div className="d-flex gap-3 flex-wrap justify-content-center mt-3">
              ...
              <Card style={{ width: "18rem", height:"100%" }}>
                <Card.Body>
                  ...
                  <Card.Text>
                    ...
                    <p>
                      ...
                      <div className="product-details__ratings">
                        ...
                        >
                          {
                             Array.from({ length: item.rating }).map((ele, ind) => {
                                   return <i class="fa-solid fa-star"></i>
                                })
                          }
                          {/* Empty Stars */}
                           {Array.from({ length: 5 - item.rating }).map((_, index) => (
                           <i key={index + item.rating} className="fa-regular fa-star"></i>
                            ))}
                        </span>
                        <span
                          className="product-details__rating-count"
                  ...
                  <Button variant="none" onClick={()=>deleteReview(item._id)}>
                    <i
